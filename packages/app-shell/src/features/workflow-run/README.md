...
- Render the Run Workspace when `workflowRunId` is selected:
  ...
  - Status language (keep it harmonious):
    ...
    - **Waiting to retry (`retry_waiting`)**: orange mark / badge / ring / path
      ...
      chips show only the short countdown. The row has no session yet, so the
      session dock shows a placeholder. It still counts as an active act, so the
      stage keeps following it, and it never counts as done in progress totals.
