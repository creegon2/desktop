...
的橙色状态，只显示「{n} 秒后开始」，读屏名称里带有次数。等待中节点的会话面板说明新一次尝试开始
后会显示它的会话。舞台像跟随运行中节点一样继续跟随等待中的节点；进度计数（「已完成 / 总数」、
迭代当轮的「x/y 完成」）不把它算作已完成。
...
所在的轮次。只有运行数据能证明时，才标注「已自动重试」或「已手动续跑」：在用行的
`auto_retry.retry` 表示紧挨在它之前有几次自动重试；一个新起的行如果尝试编号正好接在某次失败尝试
之后，就是通过续跑替换了那次尝试（编号中间有空缺，说明有一次列表不包含的取消或放弃的尝试，
更早的尝试就都不标注）。「从头重新运行」之前的尝试改标「从头重新运行前」。当前尝试保持原有显示，包括注入的
上次失败信息。被 Loop 续跑关闭的 Loop 轮次里的失败尝试不显示，因为这些轮次已经没有在用行。

**重试用尽与放弃的等待。** 在用行带有 `auto_retry.retry = n > 0` 的失败节点，会在续跑提示旁
显示「已自动重试 {n} 次，仍然失败」。以 `{"reason":"retry_abandoned"}` 取消的行显示「运行在等待
自动重试时结束，这次重试没有开始」，不再显示原始错误文本。
